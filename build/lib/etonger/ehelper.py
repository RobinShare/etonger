...
class Config(object):
    ...
    def __loadConfiguration(self):
        configPath = os.path.join(BASE_DIR, '.config/eTonger/config.xml')
        config = dict()
        with open(configPath, 'r', encoding='utf8') as fp:
            xml = fp.read()
        config = dict(xmltodict.parse(xml).get('eTonger').get('trading'))
        return config
...
class MConfig(object):
    ...
    def __loadConfiguration(self):
        ...
        with open(configPath, 'r', encoding='utf8') as fp:
            xml = fp.read()
        config = dict(xmltodict.parse(xml).get('eTonger').get('mail'))
        return config
...
class Tlog(Msg):
    ...
    def __subject(self):
        strvar = "! " + time.strftime("%H:%M:%S %Y-%m-%d", time.localtime()) + " " + str(self.__action) + " " + str(self.__status)
        strvar = re.sub(' +', ' ', strvar)
        return strvar
...
class Logging(logging.Logger):
    ...
    def __setFileHandler(self):
        path = os.path.join(BASE_DIR, 'logs/eTonger/', self.__logType)
